...
- **Signup requirements (email/password):** full name + **≥1 Kelas** (1/2/3) are required, enforced **server-side** in `/api/auth/signup` and client-side in `login.astro`. Captured into `users.reg_class1/2/3` and shown in the admin **Pending** queue + Telegram alert so admins can identify the person before linking. (Google sign-in has no form, so it doesn't collect these.)
