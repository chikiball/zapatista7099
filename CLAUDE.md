...
- **Class suggestions (crowd-sourced):** in the **directory**, any approved user can suggest a class (Kelas 1/2/3) for **another** alumni whose class field is empty. Suggestions accumulate as a ranked tally on the card (`usulan teman → 1-3 (4) · 1-5 (1)`) and display **only while the field is empty** — once the person (or admin) fills it, the confirmed value takes over. Table `class_suggestions(target_alumni_id, field, value, suggested_by_user_id, UNIQUE(target,field,by))` — **one vote per user per field** (upsert). Values validated server-side against `CLASS_OPTIONS` (dropdown enums, incl. "Lainnya" — no free text) via `suggestionsForAlumni()`. Endpoints: `POST/DELETE /api/directory/:id/suggest-class` (approved; blocks your own card + already-filled fields), `/api/directory` returns per-card `suggestions`/`my_suggestions`/`is_self` for **empty fields only**, `/api/auth/me` returns `class_suggestions` for the caller's own empty fields → **profile prompt** with one-click adopt (`POST /api/profile/adopt-suggestion` = "admit"). Works on the **~397 non-registered alumni** too (they can't self-confirm) — admin **"Usulan Kelas"** tab lists grouped suggestions with counts + suggester names and can **promote** (`POST /api/admin/alumni/:id/promote-class`) or **delete** (`DELETE /api/admin/suggestions/:id`). Public cards show counts only (not who suggested); admin sees names.
